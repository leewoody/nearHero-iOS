...
- (void)archiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)xmppStream
{
    ...
    NSString *msdId = [[message elementForName:@"id"] stringValue];
    BOOL isComposing = NO;
    ...
    [self scheduleBlock:^{
        ...
        else
        {
           ...
           // archivedMessage.body = messageBody;
            ...
            archivedMessage.body =  [messageBody substringToIndex:([messageBody length] - 32)];

            archivedMessage.encryptedId = [messageBody substringFromIndex: [messageBody length] - 32];

            archivedMessage.msgId = msdId;
            archivedMessage.isDelivered = NO;
            ...
            if ([messageBody length] > 0)
            {
                ...
                contact.mostRecentMessageBody =  [messageBody substringToIndex:([messageBody length] - 32)];
                
                contact.encryptedId = [messageBody substringFromIndex: [messageBody length] - 32];
