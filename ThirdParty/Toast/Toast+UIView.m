...
@implementation UIView (Toast)
 MMPulseView *existingActivityView;
 UIView *activityView;
...
- (void)makeToastActivityWithText:(NSString*)message {
    [self makeToastActivity:CSToastActivityDefaultPosition andText:message];
}
- (void)makeToastActivity {
    [self makeToastActivity:CSToastActivityDefaultPosition andText:nil];
...
-(void)startPulseLoading{
    self.loadingPulseView = [MMPulseView new];
    self.loadingPulseView.frame = CGRectMake(0,0,200,200);
    
    self.loadingPulseView.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionLinear];
    self.loadingPulseView.colors = @[(__bridge id)[UIColor colorWithRed:0.0 green:0.478 blue:0.984 alpha:0.7].CGColor,(__bridge id)[UIColor colorWithRed:0.0 green:0.478 blue:0.984 alpha:0.7].CGColor];
    
    CGRect screenRect = [UIScreen mainScreen].bounds;
    CGFloat posY = (CGRectGetHeight(screenRect)-320)/2/CGRectGetHeight(screenRect);
    self.loadingPulseView.startPoint = CGPointMake(0.5, posY);
    self.loadingPulseView.endPoint = CGPointMake(0.5, 1.0f - posY);
    
    self.loadingPulseView.minRadius = 10;
    self.loadingPulseView.maxRadius = 100;
    
    self.loadingPulseView.duration = 7;
    self.loadingPulseView.count = 5;
    self.loadingPulseView.lineWidth = 20.0f;
    [self.loadingPulseView startAnimation];
    [self.loadingPulseView setCenter:self.center];
    [self addSubview:self.loadingPulseView];
...
- (void)makeToastActivity:(id)position andText:(NSString*)message{
    // sanity
    CGRect screenRect = [UIScreen mainScreen].bounds;
    CGPoint center = CGPointMake(screenRect.size.width/2, screenRect.size.height/2);
    existingActivityView = [MMPulseView new];
    existingActivityView.frame = CGRectMake(0,0,200,200);
    existingActivityView.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionLinear];
     existingActivityView.colors = @[(__bridge id)[UIColor colorWithRed:0.0 green:0.478 blue:0.984 alpha:0.7].CGColor,(__bridge id)[UIColor colorWithRed:0.0 green:0.478 blue:0.984 alpha:0.7].CGColor];
    
//    CGRect screenRect = [UIScreen mainScreen].bounds;
    CGFloat posY = (CGRectGetHeight(screenRect)-320)/2/CGRectGetHeight(screenRect);
    existingActivityView.startPoint = CGPointMake(0.5, posY);
    existingActivityView.endPoint = CGPointMake(0.5, 1.0f - posY);
    
    
    existingActivityView.minRadius = 10;
    existingActivityView.maxRadius = 100;
    existingActivityView.duration = 7;
    existingActivityView.count = 5;
    existingActivityView.lineWidth = 20.0f;

    [existingActivityView startAnimation];
    existingActivityView.center = center;
    //[existingActivityView setCenter:self.center];
    //[activityView addSubview:existingActivityView];
    [self addSubview:existingActivityView];
